...
# Nueva estrategia: Sistema Factores (con ticker corregido)
SISTEMA_FACTORES = {
    "tickers": ['IS3S.DE', 'IS3Q.DE', 'IS3R.DE', 'IQQ0.DE', 'IBCZ.DE', 'VGWE.DE', 'WTEM.DE', 'IUSM.DE'] # VGWE.DE corregido
}
...
# ------------- DESCARGA (Solo CSV desde GitHub + FMP + yfinance para Sistema Factores) -------------
# Variable global para rastrear errores durante la descarga
_DOWNLOAD_ERRORS_OCCURRED = False

# --- NUEVA FUNCIÓN PARA DESCARGAR DATOS DE SISTEMA FACTORES CON YFINANCE ---
def get_yfinance_data(tickers, start, end):
    """Descarga datos usando yfinance para una lista de tickers"""
...
def download_all_data(tickers, start, end):
    ...
    # Identificar tickers del Sistema Factores
    sistema_factores_tickers = SISTEMA_FACTORES.get("tickers", [])
    tickers_para_yfinance = [t for t in tickers if t in sistema_factores_tickers]
    tickers_para_fmp_csv = [t for t in tickers if t not in sistema_factores_tickers]
    
    # Descargar tickers del Sistema Factores con yfinance
    if tickers_para_yfinance:
        st.info("📥 Descargando datos de Sistema Factores desde yfinance...")
        yfinance_data = get_yfinance_data(tickers_para_yfinance, start, end)
...
# Función corregida para Sistema Descorrelación, respetando la lógica comentada
def weights_sistema_descorrelacion(df, main, secondary):
    ...
    if len(df) >= 7:
        try:
            ...
            sig.append((df.index[-1], weights)) # Fecha final
        except Exception as e:
            sig.append((df.index[-1] if len(df) > 0 else pd.Timestamp.now(), {}))
    
    # Eliminar duplicados basados en la fecha, manteniendo el último
    seen_dates = set()
    unique_sig = []
    for date, weight_dict in reversed(sig): # Iterar en reversa
         if date not in seen_dates:
              seen_dates.add(date)
              unique_sig.append((date, weight_dict))
    unique_sig.reverse() # Volver al orden original
    
    return unique_sig if unique_sig else [(df.index[-1] if len(df) > 0 else pd.Timestamp.now(), {})]
