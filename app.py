...
def get_strategy_assets(strategy_name):
    if strategy_name == "DAA KELLER":
        return {
        ...
        return {
            "ASSETS": ['SPY', 'EFA', ' emerging_markets_placeholder']
        }
    elif strategy_name == "ABD":
        return {
            "ASSETS": ['stocks_placeholder', 'bonds_placeholder']
        }
    return {}
...
def run_daa_keller(initial_capital, start_date, end_date):
    # Definir activos
    RISKY = ['SPY', 'IWM', 'QQQ', 'VGK', 'EWJ', 'EEM', 'VNQ', 'DBC', 'GLD', 'TLT', 'HYG', 'LQD']
    PROTECTIVE = ['SHY', 'IEF', 'LQD']
    CANARY = ['EEM', 'AGG']
...
def run_combined_strategies(strategies, initial_capital, start_date, end_date, benchmark):
    ...
    for strategy in strategies:
        if strategy == "DAA KELLER":
            result = run_daa_keller(initial_capital / len(strategies), start_date, end_date)
            if result:
